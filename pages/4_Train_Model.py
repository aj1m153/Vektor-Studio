...
def apply_fe(X, fe_options, log_cols, bin_cols, bin_bins, poly_degree, interact_pairs):
    ...
    if "Binning" in fe_options:
        for c in bin_cols:
            if c in X.columns:
                X[f"bin_{c}"] = pd.cut(X[c], bins=int(bin_bins), labels=False)
    ...
    if "Polynomial features" in fe_options and poly_degree > 1:
        ...
        if num_c:
            ...
            arr     = poly.fit_transform(X[num_c])
            names   = poly.get_feature_names_out(num_c)
            poly_df = pd.DataFrame(arr, columns=names, index=X.index)
            X       = pd.concat([X.drop(columns=num_c), poly_df], axis=1)
    return X.fillna(0)

def get_estimator(module_name, class_name, params):
    mod = __import__(module_name, fromlist=[class_name])
    cls = getattr(mod, class_name)
    clean = {k: v for k, v in params.items() if v != "" and v is not None}
    return cls, clean

# ═════════════════════════════════════════════════════════════════════════════
# MODEL CATALOGUES  — single default values only (no lists)
# ═════════════════════════════════════════════════════════════════════════════
CLF_MODELS = {
    "Logistic Regression": ("sklearn.linear_model",  "LogisticRegression",         {"C": 1.0,            "max_iter": 500}),
    "Random Forest":       ("sklearn.ensemble",       "RandomForestClassifier",     {"n_estimators": 100, "max_depth": 10}),
    "Gradient Boosting":   ("sklearn.ensemble",       "GradientBoostingClassifier", {"n_estimators": 100, "learning_rate": 0.1, "max_depth": 3}),
    "XGBoost":             ("xgboost",                "XGBClassifier",              {"n_estimators": 100, "max_depth": 6,       "learning_rate": 0.1, "use_label_encoder": False, "eval_metric": "logloss"}),
    "LightGBM":            ("lightgbm",               "LGBMClassifier",             {"n_estimators": 100, "num_leaves": 31,     "verbose": -1}),
    "Decision Tree":       ("sklearn.tree",           "DecisionTreeClassifier",     {"max_depth": 5}),
    "Extra Trees":         ("sklearn.ensemble",       "ExtraTreesClassifier",       {"n_estimators": 100, "max_depth": 10}),
...
REG_MODELS = {
    ...
    "Random Forest":      ("sklearn.ensemble",     "RandomForestRegressor",      {"n_estimators": 100, "max_depth": 10}),
    "Gradient Boosting":  ("sklearn.ensemble",     "GradientBoostingRegressor",  {"n_estimators": 100, "learning_rate": 0.1, "max_depth": 3}),
    "XGBoost":            ("xgboost",              "XGBRegressor",               {"n_estimators": 100, "max_depth": 6,       "learning_rate": 0.1}),
    "LightGBM":           ("lightgbm",             "LGBMRegressor",              {"n_estimators": 100, "num_leaves": 31,     "verbose": -1}),
    "SVR":                ("sklearn.svm",          "SVR",                        {"C": 1.0,            "kernel": "rbf"}),
    "KNN":                ("sklearn.neighbors",    "KNeighborsRegressor",        {"n_neighbors": 5}),
}

# GridSearch grids — lists only used here
CLF_GRID = {
    ...
    "Extra Trees":         {"n_estimators": [50, 100], "max_depth": [5, 10, None]},
...
task = st.selectbox("Select task type", [
    "Classification", "Regression", "Clustering", "Time Series", "Prophet"
])
...
if task == "Classification":
    ...
    with st.expander("1. Data Configuration", expanded=True):
        ...
        seed      = int(c1.number_input("Random seed", 0, 9999, 42, key="clf_seed"))
    ...
    with st.expander("2. Feature Engineering", expanded=False):
        fe_options = st.multiselect("Transformations",
            ["Log transform", "Binning", "Interaction terms", "Polynomial features"], key="clf_fe")
        ...
        if "Interaction terms" in fe_options:
            n_pairs = int(st.number_input("Number of pairs", 1, 10, 1, key="clf_npairs"))
            for i in range(n_pairs):
                cc1, cc2 = st.columns(2)
                a = cc1.selectbox(f"Pair {i+1} — A", numeric_cols, key=f"clf_ia_{i}")
                b = cc2.selectbox(f"Pair {i+1} — B", numeric_cols, key=f"clf_ib_{i}")
                interact_pairs.append((a, b))
        if "Polynomial features" in fe_options:
            poly_degree = st.slider("Degree", 2, 4, 2, key="clf_poly")

    # ── 3. Model ──────────────────────────────────────────────────────────────
    with st.expander("3. Model Selection", expanded=True):
        model_name = st.selectbox("Model", list(CLF_MODELS.keys()), key="clf_model")
        module_name, class_name, default_params = CLF_MODELS[model_name]
        default_grid = CLF_GRID[model_name]

    # ── 4. Tuning ─────────────────────────────────────────────────────────────
    with st.expander("4. Hyperparameter Tuning", expanded=False):
        tuning = st.radio("Method", ["Manual", "GridSearchCV", "Optuna"],
                           horizontal=True, key="clf_tuning")
        manual_params = dict(default_params)
        ...
        if tuning == "Manual":
            ...
            for i, (pname, default_val) in enumerate(default_params.items()):
                col = pcols[i % 3]
                if pname in ("use_label_encoder", "eval_metric", "verbose"):
                    new_params[pname] = default_val
                    continue
                if isinstance(default_val, bool):
                    new_params[pname] = col.checkbox(pname, value=default_val, key=f"clf_p_{pname}")
                elif isinstance(default_val, int):
                    new_params[pname] = int(col.number_input(pname, value=int(default_val), step=1, key=f"clf_p_{pname}"))
                elif isinstance(default_val, float):
                    new_params[pname] = col.number_input(pname, value=float(default_val), step=0.01, format="%.4f", key=f"clf_p_{pname}")
                elif isinstance(default_val, str):
                    new_params[pname] = col.text_input(pname, value=default_val, key=f"clf_p_{pname}")
                elif default_val is None:
                    use_lim = col.checkbox(f"Limit {pname}?", value=False, key=f"clf_p_{pname}_use")
                    new_params[pname] = int(col.number_input(f"{pname} value", value=5, step=1, key=f"clf_p_{pname}_val")) if use_lim else None
                else:
                    new_params[pname] = default_val
            manual_params = new_params

        elif tuning == "GridSearchCV" and default_grid:
            import json
            grid_str = st.text_area("Parameter grid (JSON)", value=json.dumps(default_grid, indent=2),
                                     height=150, key="clf_grid")
            try:    param_grid = json.loads(grid_str)
            except: param_grid = default_grid; st.warning("Invalid JSON — using default grid.")

        elif tuning == "Optuna":
            n_trials = int(st.number_input("Trials", 10, 200, 30, key="clf_trials"))
    ...
    if st.button("Train Classification Model", type="primary", key="clf_train"):
        ...
        with st.spinner("Training..."):
            try:
                cls, clean_params = get_estimator(module_name, class_name, manual_params)
                ...
                if tuning == "Manual":
                    ...
                    try:
                        base = cls(random_state=seed)
                    except TypeError:
                        base = cls()
                    cv_search = GridSearchCV(base, param_grid, cv=3, n_jobs=-1, scoring="accuracy")
                    cv_search.fit(X_train, y_train)
                    st.success(f"Best parameters: {cv_search.best_params_}")
                    est = cv_search.best_estimator_

                elif tuning == "Optuna":
                    import optuna
                    optuna.logging.set_verbosity(optuna.logging.WARNING)
                    def objective(trial):
                        p = {k: trial.suggest_categorical(k, v) for k, v in default_grid.items() if v}
                        try:
                            try: m = cls(**p, random_state=seed)
                            except TypeError: m = cls(**p)
                    ...
                    for i in range(n_trials):
                        ...
                        prog.progress((i + 1) / n_trials)
                    prog.empty()
                    st.success(f"Best parameters: {study.best_params}")
                    try: est = cls(**study.best_params, random_state=seed)
                    except TypeError: est = cls(**study.best_params)
        ...
        try:
            if hasattr(est, "predict_proba"):
                ...
                y_prob_1 = y_prob[:, 1] if is_binary else y_prob
                has_prob = True
        except Exception:
            pass
        ...
        if has_prob and is_binary:
            try:
                ...
                pos_probs = y_prob_1[y_test == 1]
                neg_probs = y_prob_1[y_test == 0]
                if len(pos_probs) > 0 and len(neg_probs) > 0:
                    ks_stat, ks_pval = sp.ks_2samp(pos_probs, neg_probs)
                    somers_d = 2 * auc - 1 if auc else None
                    k1, k2 = st.columns(2)
                    k1.metric("KS Statistic", f"{ks_stat:.4f}",
                               help=f"p-value: {ks_pval:.4f}")
                    if somers_d is not None:
                        k2.metric("Somers D", f"{somers_d:.4f}")
            except Exception:
                pass
        ...
        if has_prob and is_binary:
            ...
            with col_r:
                ...
                prec_c, rec_c, _ = precision_recall_curve(y_test, y_prob_1)
                fig, ax = plt.subplots(figsize=(5, 4))
            ...
            ax.plot(thresholds, tpr_l, label="TPR (Sensitivity)")
            ax.plot(thresholds, fpr_l, label="FPR (1-Specificity)")
            ax.axvline(thresholds[ks_max_idx], color="red", linestyle="--",
                       label=f"KS = {ks_diff[ks_max_idx]:.4f}")
        ...
        if hasattr(raw, "feature_importances_"):
            ...
            ax.set_title("Feature Importances"); plt.tight_layout()
            st.pyplot(fig, use_container_width=False); plt.close()
        elif hasattr(raw, "coef_"):
            coef = np.abs(raw.coef_.flatten()[:len(feat_names)])
            fi   = pd.Series(coef, index=feat_names[:len(coef)]).nlargest(20).sort_values()
            fig, ax = plt.subplots(figsize=(6, max(3, len(fi) * 0.3)))
            fi.plot(kind="barh", ax=ax)
            ax.set_title("Coefficients (absolute)"); plt.tight_layout()
            st.pyplot(fig, use_container_width=False); plt.close()

        try:
            import shap
            X_s = pd.DataFrame(X_test[:100], columns=feat_names)
            explainer = (shap.TreeExplainer(raw) if hasattr(raw, "feature_importances_")
                         else shap.LinearExplainer(raw, X_train))
        ...
        if auc: metrics["roc_auc"] = auc
        result_entry = {"name": f"{model_name} (Classification)", "model": est,
                        "task": "Classification", "metrics": metrics,
                        "feature_cols": feat_names, "target_col": target_col}
        st.session_state.setdefault("model_results", []).append(result_entry)
        buf = io.BytesIO(); pickle.dump(est, buf)
        st.download_button("Download trained model (.pkl)", buf.getvalue(),
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander("1. Data Configuration", expanded=True):
        ...
        feature_cols = st.multiselect("Feature columns",
                                       ...
                                       key="reg_features")
        c1, c2, c3 = st.columns(3)
        seed      = int(c1.number_input("Random seed", 0, 9999, 42, key="reg_seed"))
    ...
    with st.expander("2. Feature Engineering", expanded=False):
        fe_options = st.multiselect("Transformations",
            ["Log transform", "Binning", "Interaction terms", "Polynomial features"], key="reg_fe")
        log_cols, bin_cols, bin_bins, poly_degree, interact_pairs = [], [], 4, 2, []
        if "Log transform" in fe_options:
        ...
        if "Interaction terms" in fe_options:
            n_pairs = int(st.number_input("Number of pairs", 1, 10, 1, key="reg_npairs"))
            for i in range(n_pairs):
                cc1, cc2 = st.columns(2)
                a = cc1.selectbox(f"Pair {i+1} — A", numeric_cols, key=f"reg_ia_{i}")
                b = cc2.selectbox(f"Pair {i+1} — B", numeric_cols, key=f"reg_ib_{i}")
                interact_pairs.append((a, b))
        if "Polynomial features" in fe_options:
            poly_degree = st.slider("Degree", 2, 4, 2, key="reg_poly")

    with st.expander("3. Model Selection", expanded=True):
        model_name  = st.selectbox("Model", list(REG_MODELS.keys()), key="reg_model")
        module_name, class_name, default_params = REG_MODELS[model_name]
        default_grid = REG_GRID[model_name]

    with st.expander("4. Hyperparameter Tuning", expanded=False):
        tuning = st.radio("Method", ["Manual", "GridSearchCV", "Optuna"],
                           horizontal=True, key="reg_tuning")
        manual_params = dict(default_params)
        ...
        if tuning == "Manual":
            ...
            for i, (pname, default_val) in enumerate(default_params.items()):
                col = pcols[i % 3]
                if pname == "verbose":
                    new_params[pname] = default_val; continue
                if isinstance(default_val, bool):
                    new_params[pname] = col.checkbox(pname, value=default_val, key=f"reg_p_{pname}")
                elif isinstance(default_val, int):
                    new_params[pname] = int(col.number_input(pname, value=int(default_val), step=1, key=f"reg_p_{pname}"))
                elif isinstance(default_val, float):
                    new_params[pname] = col.number_input(pname, value=float(default_val), step=0.01, format="%.4f", key=f"reg_p_{pname}")
                elif isinstance(default_val, str):
                    new_params[pname] = col.text_input(pname, value=default_val, key=f"reg_p_{pname}")
                elif default_val is None:
                    use_lim = col.checkbox(f"Limit {pname}?", value=False, key=f"reg_p_{pname}_use")
                    new_params[pname] = int(col.number_input(f"{pname} value", value=5, step=1, key=f"reg_p_{pname}_val")) if use_lim else None
                else:
                    new_params[pname] = default_val
            manual_params = new_params
            ...
            import json
            grid_str = st.text_area("Parameter grid (JSON)", value=json.dumps(default_grid, indent=2),
                                     height=150, key="reg_grid")
            try:    param_grid = json.loads(grid_str)
            except: param_grid = default_grid

        elif tuning == "Optuna":
            n_trials = int(st.number_input("Trials", 10, 200, 30, key="reg_trials"))

    with st.expander("5. Regression Diagnostics", expanded=False):
        run_vif      = st.checkbox("VIF (multicollinearity)", value=True, key="reg_vif")
        run_dw       = st.checkbox("Durbin-Watson (autocorrelation)", value=True, key="reg_dw")
        run_bp       = st.checkbox("Breusch-Pagan (heteroscedasticity)", value=True, key="reg_bp")
        run_normtest = st.checkbox("Normality test on residuals", value=True, key="reg_norm")
        run_qq       = st.checkbox("QQ plot", value=True, key="reg_qq")
    ...
    if st.button("Train Regression Model", type="primary", key="reg_train"):
        ...
        with st.spinner("Training..."):
            try:
                ...
                if tuning == "Manual":
                    ...
                    try: base = cls(random_state=seed)
                    except TypeError: base = cls()
                    cv_search = GridSearchCV(base, param_grid, cv=3, n_jobs=-1,
        ...
        if run_normtest:
            try:
                if len(residuals) <= 5000:
                    ...
                    diag["Shapiro-Wilk (normality)"] = f"W={stat:.4f}, p={pval:.4f} — {'Normal' if pval > 0.05 else 'Not normal'}"
                else:
                    stat, pval = sp.normaltest(residuals)
                    diag["D'Agostino (normality)"] = f"Stat={stat:.4f}, p={pval:.4f} — {'Normal' if pval > 0.05 else 'Not normal'}"
            except Exception as e:
                diag["Normality test"] = f"Could not compute: {e}"

        if run_dw:
            try:
                from statsmodels.stats.stattools import durbin_watson
                dw = durbin_watson(residuals)
                diag["Durbin-Watson"] = f"{dw:.4f} — {'No autocorrelation' if 1.5 < dw < 2.5 else 'Possible autocorrelation'}"
            except Exception as e:
                diag["Durbin-Watson"] = f"Could not compute: {e}"

        if run_bp:
            try:
                from statsmodels.stats.diagnostic import het_breuschpagan
                bp_lm, bp_pval, _, _ = het_breuschpagan(residuals, X_test.fillna(0))
                diag["Breusch-Pagan"] = f"LM={bp_lm:.4f}, p={bp_pval:.4f} — {'Homoscedastic' if bp_pval > 0.05 else 'Heteroscedastic'}"
            except Exception as e:
                diag["Breusch-Pagan"] = f"Could not compute: {e}"
        ...
        if run_vif:
            try:
                ...
                X_vif = X_train.select_dtypes(include=np.number).fillna(0)
                vif_df = pd.DataFrame({
        ...
        if hasattr(raw, "feature_importances_"):
            ...
            coef = pd.Series(raw.coef_.flatten()[:len(feat_names)],
                             index=feat_names[:len(raw.coef_.flatten())]).sort_values()
            fig, ax = plt.subplots(figsize=(6, max(3, len(coef) * 0.3)))
            coef.plot(kind="barh", ax=ax); ax.set_title("Coefficients"); plt.tight_layout()
            st.pyplot(fig, use_container_width=False); plt.close()

        result_entry = {"name": f"{model_name} (Regression)", "model": est,
                        "task": "Regression",
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander("1. Data Configuration", expanded=True):
        feature_cols = st.multiselect("Feature columns", all_cols,
                                       default=numeric_cols, key="clu_features")
        scale = st.checkbox("Scale features", True, key="clu_scale")

    with st.expander("2. Algorithm and Parameters", expanded=True):
        algo = st.selectbox("Algorithm", ["KMeans", "DBSCAN", "Agglomerative"], key="clu_algo")
        if algo == "KMeans":
            k    = st.slider("Number of clusters (k)", 2, 15, 3, key="clu_k")
            seed = int(st.number_input("Random seed", 0, 9999, 42, key="clu_seed"))
        elif algo == "DBSCAN":
            eps         = st.number_input("eps", 0.1, 10.0, 0.5, key="clu_eps")
            min_samples = int(st.number_input("min_samples", 1, 50, 5, key="clu_min"))
        else:
    ...
    if st.button("Run Clustering", type="primary", key="clu_train"):
        ...
        sc = ax.scatter(coords[:, 0], coords[:, 1], c=labels, cmap="tab10", alpha=0.7, s=20)
        ax.set_title("Clusters — PCA 2D Projection")
        ax.set_xlabel("PC1"); ax.set_ylabel("PC2")
        plt.colorbar(sc, ax=ax, label="Cluster"); plt.tight_layout()
        st.pyplot(fig, use_container_width=False); plt.close()

        st.markdown("**Cluster Profile Table**")
        df_clustered           = df[feature_cols].copy()
        df_clustered["Cluster"] = labels
        num_feat = [c for c in feature_cols if pd.api.types.is_numeric_dtype(df[c])]
        if num_feat:
            profile = df_clustered.groupby("Cluster")[num_feat].mean().round(3)
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander("1. Data Configuration", expanded=True):
        ...
        test_periods     = st.slider("Test periods (held out)", 3, 30, 6, key="ts_test")
    ...
    with st.expander("2. Model Selection", expanded=True):
        ...
        if model_type == "SARIMA":
            order_mode = st.radio("Order selection", ["Manual", "Automatic (AIC search)"],
                                   horizontal=True, key="ts_order")
            if order_mode == "Manual":
                c1, c2, c3 = st.columns(3)
                p = int(c1.number_input("p", 0, 5, 1, key="ts_p"))
                d = int(c2.number_input("d", 0, 2, 1, key="ts_d"))
                q = int(c3.number_input("q", 0, 5, 1, key="ts_q"))
                c4, c5, c6, c7 = st.columns(4)
                P = int(c4.number_input("P", 0, 3, 0, key="ts_P"))
                D = int(c5.number_input("D", 0, 2, 0, key="ts_D"))
                Q = int(c6.number_input("Q", 0, 3, 0, key="ts_Q"))
                s = int(c7.number_input("s", 1, 52, 12, key="ts_s"))
    ...
    if st.button("Fit and Forecast", type="primary", key="ts_train"):
        ...
        with st.spinner("Fitting model..."):
            try:
                if model_type == "SARIMA":
                    if order_mode == "Automatic (AIC search)":
                        best_aic, best_order = np.inf, (1, 1, 1)
                        combos = [(pi, di, qi) for pi in range(3)
                                  for di in range(2) for qi in range(3)]
                        prog = st.progress(0, text="Searching orders...")
                        for i, (pi, di, qi) in enumerate(combos):
                            try:
                                res = SARIMAX(train_series, order=(pi, di, qi),
                        ...
                        p, d, q = best_order
                        P, D, Q, s = 0, 0, 0, 0
                        st.success(f"Best order: {best_order} (AIC={best_aic:.2f})")
        ...
        pd.Series(test_pred.values[:len(test_series)],
                  index=test_series.index).plot(ax=ax, label="Predicted (test)",
        ...
        if model_type == "SARIMA":
            ...
            axes[1].hist(resid, bins=30, edgecolor="none")
...
# ═════════════════════════════════════════════════════════════════════════════
    ...
    with st.expander("2. Prophet Parameters", expanded=True):
        ...
        seasonality_mode   = st.selectbox("Seasonality mode", ["additive", "multiplicative"], key="pr_sm")
